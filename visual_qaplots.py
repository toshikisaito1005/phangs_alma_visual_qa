...
###extract diagnostic plots from delivered calibrated visibility data for PHANGS
#2018-02-07: developed by tsaito
#2018-02-08: add ImageMajick commands (png2pdf)
#
#
###How to run
#Simply locate this script under */2017.1.00886.L/ directory where all science_*
#direcitories are located. This script will create visual_qa/ directory under
#*/2017.1.00886.L/ which contains all png files. The name of the created sub-dir
#-ectories is same as the *.ms name. When you want to re-run this script for a
#certain ms (e.g., uid___A002_Xc55c89_X426c.ms), you need to remove a sub-direct
#-ory, visual_qa/uid___A002_Xc55c89_X426c/, in advance. Re-run for all ms, just
#remove visual_qa/ directory.
...
#for i in [0,1]:
    ...
    dir_save = "../visual_qa/" \
               + dir_ms[i].split("/")[5].replace(".ms", "") + "/"
    print("png2pdf: " + dir_ms[i].split("/")[5])
    ...
    tsys_file = glob.glob(dir_weblog[0] + "html/stage7/tsys-" \
                          + dir_ms[i].split("/")[5] + "-summary.spw*.png")
    ...
    for m in range(len(field_intent)):
        ...
        os.system("montage " + other_file[0] + " " + other_file[1] + " " \
                  ...
                  + dir_save + dir_ms[i].split("/")[5].replace(".ms", "") \
                  + "_status.pdf")
        #plotms png files
        for n in ["amp", "phase"]:
            print("test " + field_intent[m] + " " + n)
            #ant1
            ...
            os.system("montage " + im[0] + " " + im[1] + " " + im[2] + " " \
                      + im[3] + " -tile 2x2 -geometry 800x600+10+10 " \
                      + dir_save + "p001.pdf")
            #ant2
            im = sorted(glob.glob(dir_save \
                        + dir_ms[i].split("/")[5].replace(".ms", "") \
            ...
            im = sorted(glob.glob(dir_save \
                        + dir_ms[i].split("/")[5].replace(".ms", "") \
                        + "_" + n + "*freq*" + field_intent[m] + "*.png"))
            im2 = sorted(glob.glob(dir_save \
                         + dir_ms[i].split("/")[5].replace(".ms", "") \
                         + "_" + n + "*time*" + field_intent[m] + "*.png"))
            im.extend(im2)
            os.system("montage " + im[0] + " " + im[1] \
                      + " -tile 2x1 -geometry 800x600+10+10 " + dir_save \
                      + "p004.pdf")
            #merge
